...
def draw_grid():
    ...
    for c in range(COLS + 1):
        pygame.draw.line(screen, WHITE, (c * TILE_SIZE -
                         scroll[0], 0), (c * TILE_SIZE - scroll[0], SCREEN_HEIGHT))
    # horizontal lines
    for c in range(ROWS + 1):
        pygame.draw.line(screen, WHITE, (0, c * TILE_SIZE -
                         scroll[1]), (SCREEN_WIDTH, c * TILE_SIZE - scroll[1]))
...
def draw_world():
    for y, row in enumerate(world_data):
        for x, tile in enumerate(row):
            if tile >= 0:
                screen.blit(img_list[tile], (x * TILE_SIZE -
...
while run:
    ...
    # draw bottom panel
    pygame.draw.rect(screen, BG, (0, SCREEN_HEIGHT,
                     SCREEN_WIDTH + SIDE_MARGIN, SCREEN_HEIGHT + LOWER_MARGIN))
    ...
    if os.path.exists(f'level{level}_data.csv'):
        draw_text('File already exists, be careful when saving!',
                  font, WHITE, 400, SCREEN_HEIGHT + LOWER_MARGIN - 80)
